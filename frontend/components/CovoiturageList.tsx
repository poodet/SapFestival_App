...
interface CovoiturageListProps {
  hideHeader?: boolean;
}

export default function CovoiturageList({ hideHeader = false }: CovoiturageListProps) {
  ...
  const handleCreate = async () => {
    ...
    if (!totalSeats || totalSeats.trim() === '' || isNaN(parseInt(totalSeats)) || parseInt(totalSeats) < 0) {
  ...
  const handleUpdate = async () => {
    ...
    if (!conductorName.trim() || !totalSeats || parseInt(totalSeats) < 0) {
    ...
    try {
      ...
      await updateCovoiturage(editingCovoiturage.id, {
        conductorName: conductorName.trim(),
        totalSeats: parseInt(totalSeats),
        reservedSeats: editingCovoiturage.reservedSeats,
        departureDate,
        departureLocation: departureLocation.trim(),
        arrivalLocation: arrivalLocation.trim(),
        tripType: formTripType,
